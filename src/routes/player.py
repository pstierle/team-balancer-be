...
@auth_guard()
def get_players():
    players = db.session.query(Player).join(Game).filter(
        Player.user_id == request.environ["user_id"]).all()
...
def update_player():
    ...
    for game in player.games:
        updated_game = next(
            filter(lambda n: n['id'] == game.id, body['games']), None)
...
def create_player():
    ...
    if request.environ.get('user_id'):
        player = Player(name=request.json["name"],
                        user_id=request.environ.get('user_id'), id=str(uuid.uuid4()))
        db.session.add(player)
        db.session.commit()

        for base_game in base_games:
            game = Game(elo=0, player_id=player.id,
                        base_game_id=base_game['id'], id=str(uuid.uuid4()))
            db.session.add(game)

        db.session.commit()
        return serialize_player(Player.query.filter(Player.id == player.id).join(Game).first())
